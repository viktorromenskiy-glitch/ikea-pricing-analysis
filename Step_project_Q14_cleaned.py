...
def check_old_price_leakage(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if 'price' not in df.columns or 'old_price' not in df.columns:
        raise KeyError("В DataFrame должны присутствовать колонки 'price' и 'old_price'.")
    ...
    total_len = len(df)
    check_len = len(df_check)

    print(f"\n📊 Статистика old_price:")
    print(f"  • Всего товаров: {total_len}")
    print(f"  • Товаров со старой ценой: {check_len} ({check_len / total_len * 100:.1f}%)")
    print(f"  • Товаров без старой цены: {total_len - check_len} ({(total_len - check_len) / total_len * 100:.1f}%)")

    if check_len < 2:
        print("⚠️ Слишком мало данных со старой ценой для корректного анализа.")
        return {'verdict': 'SAFE', 'features_to_use': ['has_old_price'], 'features_to_avoid': ['old_price']}

    # ========================================================================
    # ПРОВЕРКА 1: Корреляция
    # ========================================================================
    ...
    # ========================================================================
    # ПРОВЕРКА 2: Отношение old_price / price
    # ========================================================================
    ...
    print(f"  • Средняя скидка: {discount_mean:.2f}%")
    print(f"  • Std скидки:     {discount_std:.2f}%")

    # ========================================================================
    # ПРОВЕРКА 4: Проверка функциональной зависимости
    # ========================================================================
    is_formula = ratio_mean > 1.15 and ratio_mean < 1.35 and ratio_std < 0.05

    # ИТОГОВЫЙ ВЕРДИКТ (согласованный с логикой формулы)
    if corr > 0.98 or is_formula:
    ...
    # [0, 0] Scatter
    ax1 = axes[0, 0]
    ax1.scatter(df_check['old_price_parsed'], df_check['price'], alpha=0.5, s=20, c='steelblue')
    max_val = max(df_check['old_price_parsed'].max(), df_check['price'].max())
    ax1.plot([0, max_val], [0, max_val], 'r--', label='y = x')

    # Проверка на корректность polyfit (избегаем инфинитов)
    if not np.isnan(corr) and check_len > 1:
        slope, intercept = np.polyfit(df_check['old_price_parsed'], df_check['price'], 1)
        ax1.plot(np.linspace(0, max_val, 100), slope * np.linspace(0, max_val, 100) + intercept, 'g-',
    ...
    # [0, 1] Ratio Histogram
    ax2 = axes[0, 1]
    ax2.hist(df_check['ratio'].dropna(), bins=50, color='coral', alpha=0.7)
    ax2.axvline(ratio_mean, color='red', linestyle='--')
    ax2.grid(True, alpha=0.3)

    # [1, 0] Discount Histogram
    ax3 = axes[1, 0]
    ax3.hist((df_check['discount_pct'] * 100).dropna(), bins=50, color='gold', alpha=0.7)
    ax3.axvline(discount_mean, color='red', linestyle='--')
    ax3.grid(True, alpha=0.3)

    # [1, 1] Boxplot (Исправлен tick_labels -> labels)
    ax4 = axes[1, 1]
    if 'category' in df_check.columns and df_check['category'].nunique() > 0:
        top_cats = df_check['category'].value_counts().head(5).index
        df_top = df_check[df_check['category'].isin(top_cats)]
        data_to_plot = [df_top[df_top['category'] == cat]['discount_pct'].values * 100 for cat in top_cats]

        # ИСПОЛЬЗУЕМ labels ВМЕСТО tick_labels
        bp = ax4.boxplot(data_to_plot, labels=[str(c)[:15] for c in top_cats], patch_artist=True)
        for patch in bp['boxes']:
            patch.set_facecolor('lightblue')
        plt.setp(ax4.get_xticklabels(), rotation=45, ha='right')
        ax4.grid(True, alpha=0.3, axis='y')
    else:
        ax4.text(0.5, 0.5, 'Нет данных о категориях', ha='center', va='center', transform=ax4.transAxes)

    plt.tight_layout()
    try:
        save_plot(fig, 'eda_old_price_leakage_check')
    except NameError:
        plt.savefig('eda_old_price_leakage_check.png')
    plt.close()
    ...
    return {
        ...
        'n_with_old_price': check_len,
        'n_without_old_price': total_len - check_len,
        'pct_with_old_price': check_len / total_len * 100,
...
def prepare_ml_data(
   ...
   Выполняет комплексную подготовку и конструирование признаков (Feature Engineering)
    ...
    df_ml['desc_quality_score'] = df_ml['short_description'].str.lower().apply(
    ...
    print(f"✓ desc_quality_score: товаров с премиум-маркерами = {(df_ml['desc_quality_score'] > 0).sum()}")
    ...
    print(f"✓ has_discount: {df_ml['has_discount'].sum()} товаров")
    ...
    # Оставляем только одно взаимодействие: сложность × премиум
    df_ml['complexity_x_premium'] = (
            df_ml['assembly_complexity'] * df_ml['is_premium_category']
    )
    print(f"✓ complexity_x_premium: взаимодействие сложность × премиум")
    ...
    print(f"❌ complexity_x_log_volume, is_complex_assembly УДАЛЕНЫ (мультиколлинеарность)")
    ...
    for i, col in enumerate(X.columns, 1):
        ...
        print(f"  {i:2d}. {col:30s} | тип: {str(dtype):10s} | пропусков: {n_missing:4d}")
...
def analyze_feature_importances(
    ...
    # Списки признаков для доменной классификации и покраски бизнес-графика
    categorical_names = ['category']
    bool_names = ['sellable_online']
    binary_names = [
        'is_team', 'has_other_colors', 'is_composite', 'has_old_price',
        'has_depth', 'has_height', 'has_width', 'is_premium_category', 'is_large_item'
    ]

    # Все остальные колонки динамически считаются числовыми (габариты, NLP, discount_abs)
    numeric_names = [col for col in X_data.columns if col not in categorical_names + bool_names + binary_names]

    # Переменная-индикатор формата имен (с префиксами sklearn или сырые из DataFrame)
    is_preprocessed_names = False
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        try:
            ...
            is_preprocessed_names = True
        except Exception:
            feature_names = [f"num__feature_{i}" for i in range(len(importances))]
            is_preprocessed_names = True
    ...
    # Защитная проверка размерностей перед сборкой DataFrame
    if len(feature_names) != len(importances):
        print(f"⚠️ Предупреждение: несовпадение размерностей ({len(feature_names)} фичей vs {len(importances)} весов).")
    ...
    def get_base_name(fname: str) -> str:
        """Определяет базовое имя признака для бизнес-группировки."""
        fname = str(fname)
        if not is_preprocessed_names:
            return fname
        ...
        # Бинарные признаки: bin__is_team -> is_team
            ...
            return fname[5:]
        # Обработка неописанных категориальных признаков
    ...
    # Функция определения макро-типа фичи
    def get_type(fname: str) -> str:
        base_name = get_base_name(fname)
        if base_name in categorical_names or fname.startswith('cat__'):
            return 'Категория товара'
        elif base_name in binary_names or base_name in bool_names or fname.startswith('bin__'):
            return 'Бинарные признаки'
        return 'Числовые (габариты + NLP)'

    importance_df['Type'] = importance_df['Feature'].apply(get_type)

    # Сгруппированная важность по макро-типам признаков
    type_importance = importance_df.groupby('Type')['Importance_%'].sum().sort_values(ascending=False)

    # Очищенная бизнес-таблица (схлопывание подкатегорий)
    ...
    # Перенормализуем бизнес-профиль к 100%
    ...
    grouped_sorted = grouped_importance_df.iloc[::-1]

    # Подбор цветов по доменным типам фичей
    colors = []
    ...
    # Отрисовка текстовых процентов поверх столбцов
...
def check_data_leakage(
  ...
  Проверяет модель на наличие утечки данных (Data Leakage) через целевые или синтетические признаки.
    ...
    2.  Признаки группы `old_price` (`has_old_price`, `discount_pct`, `discount_abs`), которые
    ...
    # Проверяем, есть ли признаки из old_price в данных
...
def gridsearch_best_model(
    ...
    Note:
        🔧 ИСПРАВЛЕНИЕ: удалена конструкция `with parallel_backend('threading')`,
        которая вызывала десятки предупреждений sklearn. Теперь используется только
        `n_jobs=-1` в GridSearchCV, что корректно задействует все ядра процессора
        без конфликтов с внутренним механизмом параллелизации sklearn.
    """
